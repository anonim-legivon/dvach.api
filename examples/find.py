...
matched_threads = api.find(patterns=['webm', 'fap', 'фап'], antipatterns=['trap', 'трап', 'black'])
...
matched_posts = sum([api.find(thread=thread.num, patterns=['соус']) for thread in matched_threads], [])
